...
//
// Static fullscreen fragment shader for the hero backdrop — a smooth gold/amber "mesh
// gradient" (ADR-0021, was blue): soft-edged gaussian dark blobs overlapping into one
// near-black mass over a gold field, with a bright bottom-left glow and a softer
// top-right glow. No grain (kept smooth for contrast). Reproduces a supplied reference.
// It renders once (and on resize / colour change) — no render loop — and sits behind
// the Plasma Burst canvas, which composites onto it via `mix-blend-lighten`. `uBase` +
// `uLight` are configurable (panel-driven); the blob shapes/positions AND the `navy` /
// `ltblue` accent colours below are baked into the shader source, not uniforms — edit
// them directly to retheme.
...
void main(){
  ...
  vec3 navy   = vec3(0.035, 0.020, 0.008); // deep warm brown-black (keeps a gold hue, not grey)
  vec3 ltblue = vec3(1.0, 0.78, 0.38);      // warm amber glow (was a saturated sky-blue)

  // Base: vivid gold, only a gentle vertical falloff (stays saturated).
  ...
  // Punch: boost saturation + a touch of contrast so mid-tones read as the base hue, not grey.
