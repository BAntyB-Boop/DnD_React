...
The metadata generator, `robots.ts`, `sitemap.ts`, and the JSON-LD helper all read
from it. Wired to the **Stride** brand ("Modern banking for people and businesses")
— `name: "Stride"`, a `tagline` field (used for the homepage `<title>` and the
Organization `slogan`), a fintech description, `themeColor: #04070f` (the dark hero
backdrop the page opens on).
...
`Organization` (name, `description`, `slogan`, url, logo) + `WebSite` JSON-LD
graph, linked by `@id`. The root layout renders it once in a
