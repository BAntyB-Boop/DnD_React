...
- **Works cards resized to 3:4 portrait + continuous strip** (see ADR-0023). The
  ...
  ratio exactly — zero cropping) and the cylinder `STEP` went 27→29deg so adjacent
  cards overlap by a sliver mid-transition. The pinned section heading gained a soft
  text-shadow for legibility while cards pass behind it. Verified via `next build` +
  `lint` + a Playwright scroll sequence.

- **Full setting pivot: "The Veiled Codex" → "The Starbound Codex"** (see ADR-0022).
  Swapped the entire fantasy campaign for an original Spelljammer-style space pantheon —
  seven gods born from a single "Sundering," ruling the Drift from the free port Nova
  Stellare. Brought in 10 AI-generated illustrations (7 god portraits, 3 world scenes).
  Renamed routes (`/hero`→`/pantheon`, `porto-stellare`→`nova-stellare` under `/lore`
  `/map`, `porto-arrival`→`nova-arrival` under `/oneshot`). Rewrote `data/mocks/home.ts`
  and `data/mocks/story.ts` from scratch (Prologue = the Sundering + 7 bilingual EN/TH
  god-origin chapters, reusing the existing per-chapter-accent + intrinsic-image-size
  patterns from ADR-0018/0019 unchanged). Updated branding
  (`siteConfig`/`manifest.json`/`package.json`). Deleted all now-unused fantasy assets
  (`public/assets/characters/`, old world/story images) and the fully-orphaned legacy
  Stride fintech stock photos under `public/assets/images/`. Verified via `next build` +
  `lint` + a full Playwright route sweep (no console errors, no failed requests).

- **Retinted the WebGL scenes from blue to gold** (see ADR-0021). The home page
  background was still visibly blue after the CSS token reskin, because the actual
  backdrop is a WebGL shader (`lib/three/gradient-background-scene.ts`), not the CSS
  `hero-gradient` fallback. Retinted `plasma-burst-scene.ts`'s `defaultPlasmaConfig`,
  `chain.tsx`/`footer-scene.tsx`'s `BASE`/`LIGHT` constants, and
  `chain-scene.ts`'s chrome material tint. Also found two colours hardcoded directly in
  the GLSL fragment shader source (not uniforms) — `navy` and `ltblue` in
  `gradient-background-scene.ts` — which is why the top-right corner stayed blue even
  after the config changes; edited the shader source itself. Hero burst, chain link, and
  footer heart now render gold/black end to end. Verified via `next build` + `lint` + a
  Playwright screenshot pass (no console errors).

- **QA pass: removed leftover fintech imagery, wired 3 dead CTAs** (see ADR-0020). Found
  and removed hardcoded stock images the reskin missed (they weren't props-driven, so the
  content pass never touched them): a stock photo of people in `stats.tsx`'s collab card
  and 4 stock avatar headshots under its quote, and the old Stride brand's blue 3D render
  in `product.tsx`'s first card — all replaced with solid colour blocks matching their
  sibling cards. Converted `showcase.tsx` "Enter the archive", `works.tsx` "Read folio",
  and `product.tsx` "Join a session" from inert `<button>`s to content-driven `next/link`
  `<Link>`s (`ctaHref`/`viewHref` fields), pointing at `/hero` and
  `/oneshot/porto-arrival`. Full-site audit (every route, desktop + mobile viewport,
  console/network checks, click-through nav test) found no other console errors, failed
  requests, or broken links.

- **Per-chapter accent colors + image crop fixes** (see ADR-0019). `/story` chapters each
  get a mood-matched accent hex (`data/mocks/story.ts`), applied to the number label,
  roman numeral, and image caption/border. Removed the Home Showcase's mislabeled
  "Bestiary" card (was a hero portrait, not a monster — no real bestiary asset exists
  yet); swapped "Chronicle" to the real `ch1-tavern.jpg` (now shared with `/story`);
  deleted the now-unused `kael-climax-bottle.png`; `showcase.tsx` grid is `lg:grid-cols-3`
  to match. Fixed character-portrait cropping in `works.tsx` (`object-top`, so tall
  portraits keep the face instead of a centre-cropped torso slice) and in `/story`
  (chapter images now render at their real intrinsic aspect ratio via `next/image`
  width/height, no forced `aspect-[16/9]` crop). Verified via `next build` + `lint` +
  a Playwright pass (no console errors).

- **Built `/story` — "Chronicle of Borderland"** (see ADR-0018). Full bilingual EN/TH
  prologue + 6 chapters transcribed from the source `story.html`, rebuilt with this
  template's own primitives (`<Inview mode="once">` reveals, `<AnimatedHeading>` chapter
  titles) rather than porting the original's bespoke cosmic-scroll intro/3D galaxy/film
  grain. New: `data/mocks/story.ts` (`StoryChapter`/`StoryBlock` union for prose/list/
  image blocks), `hooks/use-story-language.ts` (Zustand EN/TH store), `views/story.tsx`
  (Server Component) + client leaves `story-header.tsx` / `story-chapter.tsx` /
  `story-closing.tsx` / `story-language-toggle.tsx`. Extracted the shared nav content out
  of `data/mocks/home.ts` into `data/mocks/site-nav.ts` so `/` and `/story` don't
  duplicate it — `<SiteNav>` is now shown on every route (the source site used a minimal
  `.back-link` on inner pages instead). Copied the 7 chapter images into
  `public/assets/story/`. Added a `/sessions/session-1` placeholder route (the closing
  CTA's target). Verified via `next build` + a Playwright pass exercising the language
  toggle (no console errors).

- **Reskinned the starter to "The Veiled Codex"** (D&D 5e campaign site) — see ADR-0017.
  Retinted every design token in `app/globals.css` from the fintech blue/purple palette to
  a gold/black/parchment gothic palette; swapped Mulish for Cinzel (headings) + Cormorant
  Garamond (body) via `next/font/google`; rewrote all home-page content
  (`data/mocks/home.ts`, `product.ts`, `contact.ts`) with real Codex copy (hero, roster of
  seven folio characters, stats, footer); added a shared `components/ui/sigil-icon.tsx`
  brand mark used by `SiteNav` and `Preloader` (the latter had "Stride" hardcoded, not a
  content prop); `SiteNav` now takes `{label, href}[]` and navigates with real
  `next/link`s instead of inert `href="#"`. Added six placeholder routes (`/hero`,
  `/story`, `/bestiary`, `/lore/porto-stellare`, `/map/porto-stellare`,
  `/oneshot/porto-arrival`) via a shared `views/coming-soon.tsx` so the new nav doesn't
  404. Copied character card art + world art into `public/assets/characters/<slug>/` and
  `public/assets/world/` from the source project. Added `images.qualities: [30, 75]` to
  `next.config.ts` (the Works cards use `quality={30}`, which Next 16 now requires to be
  explicitly allow-listed). `siteConfig` (`lib/site.ts`) and `public/manifest.json`
  updated to match. Verified via `next build` + a Playwright screenshot pass (no console
  errors). Not yet retinted: the WebGL scene shader colours (hero plasma burst, chain) —
  those are artwork constants per ADR-0014, not tokens.
